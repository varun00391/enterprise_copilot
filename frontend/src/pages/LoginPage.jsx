...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (oe === 'oauth_failed') {
      setError(msg?.trim() || 'Google sign-in did not finish. Try again or use email/password.')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(user.role === 'super_admin' || user.role === 'dept_admin' ? '/admin' : '/dashboard')
    } catch (err) {
    ...
    <div className="min-h-screen bg-navy-800 flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-md animate-slide-up">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400">Sign in to your department workspace</p>
        </div>
        ...
        <p className="text-center mt-4 text-sm text-gray-600">
          Platform admin?{' '}
          <Link to="/admin/login" className="text-gray-500 hover:text-gray-400">
            Admin portal →
          </Link>
        </p>
      </div>
